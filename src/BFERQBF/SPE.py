...
def PPRF_punc(t, sk, sec_parameter=128):
    ...
    Returns:
    psk：穿刺密钥，一个list容器，其中包含2个元素。第一个元素储存sk，第二个list储存被刺穿的标签t     
    '''
    # 将t转换为01字符串
    # t = bin2str(t)
...
def Enc(msk_0, msg, t):
    ...
    ct = AES.new(key=int(1022).to_bytes(16,'big',signed=True), mode=AES.MODE_ECB).encrypt(msg)
...
def Dec(SK, ct, t):
    ...
    msg=AES.new(key=int(1022).to_bytes(16,'big',signed=True), mode=AES.MODE_ECB).decrypt(ct)
