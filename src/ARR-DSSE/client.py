...
def construct_inverted_index(inverted_index,map_r):
    '''
    ...
    map_r：一个字典容器，储存w->[r]，r是16字节长的随机bytes数组

    Parameters:
    inverted_index：一个字典容器，储存w->[ind,r]，这里w是普通的字符串，如'5'，'8'；ind为一个列表

    Returns:
    kw_files_list：一个字典容器，储存w->[ind||r]，这里w是16字节长的标准字符串，其中包含'*'；ind||r是32字节长的bytes数组
    '''
    kw_files_list = {}
    ...
    for w in inverted_index:
        ...
        # 判断w是否已经有r，没有的话随机生成一个
        if s not in map_r:
            # 随机生成一个16位的r
            r = random.randint(-sys.maxsize-1, sys.maxsize)
            r = hmac.new(int(r).to_bytes(
        ...
        # 对索引列表中每个ind，计算其ind||r储存在list中
        l=[]
        for ind in inverted_index[w]:
            l.append(ind.zfill(16).encode('utf-8')+r)   
        ...
        for i in range(-pos):
            ...
            if s in kw_files_list:
                kw_files_list[s] = merge(kw_files_list[s], l)
            else:
                kw_files_list[s] = copy.deepcopy(l)            
...
def build_client(K_e, K_s, inverted_index):
    ...
    Returns:
    state_client：client端的状态表，一个字典容器，储存kw->[c,v,gamma_add_0,h,msk_c]
    map_server：server端的储存容器，将addr映射到[P,V]
    checklist:一个字典，储存lw->h    
    MSK_0：一个字典，储存w->msk_0
    map_r：一个字典。储存所有叶结点对应的w的r值
    '''
    state_client = {}
    map_server = {}
    checklist = {}
    ...
    # kw_files_list为PTKS上叶结点以及其所有祖先结点的反转索引，且关键词为标准字符串形式，值为ind||r
    # 同时顺便生成所有叶结点对应的w的r
    kw_files_list=construct_inverted_index(inverted_index,map_r)
    ...
    for kw in kw_files_list:
        ...
        # 随机选择一个gamma_add_0。先生成一个随机数，然后求这个随机数的哈希
        r_temp = random.randint(-sys.maxsize-1, sys.maxsize)
        gamma_add_0 = hmac.new(int(r_temp).to_bytes(
            length=16, byteorder='big', signed='True')).digest()
        ...
        # 生成SPE的主密钥msk_0，设置最大穿刺次数为50
        msk_0=SPE.KeyGen(sec_parameter=128,d=5001)
        ...
        # 遍历该w对应的所有文件id
        # 当前的gamma
        gamma_now = gamma_add_0
        # 这里的ind实质上是ind||r
        for ind in kw_files_list[kw]:

            # 生成新的gamma_next
            r_temp = random.randint(-sys.maxsize-1, sys.maxsize)
            gamma_next = hmac.new(int(r_temp).to_bytes(
                length=16, byteorder='big', signed='True')).digest()
            ...
            # 密文的第二部分 Enc(K_w,ind||r)
            val = Enc.encrypt(ind)  # 32字节的密文


            # 计算val的tag
            t=hmac.new(val).digest()

            
            # 计算ct=SPE.encrypt(msk_0,val,t)
            ct=SPE.Enc(msk_0,val,t)


            # I.put(addr,(P,ct,t))
            map_server[addr] = [pos, ct,t]
        ...
        # 更新状态表
        state_client[kw] = [c, v, gamma_add_0, h,msk_0]

        # 更新MSK_0表，储存每个w的msk_0
        MSK_0[kw]=msk_0
        

    # 返回三个字典容器
    return state_client, map_server, checklist,MSK_0,map_r


def token_client(lb, ub, state_client, K_s, MSK_0,map_del):
    '''
    ...
    K_s：密钥
    MSK_0：储存每个w的初始SPE主密钥 msk_0
    map_del：一个字典，储存w对应的被穿刺的gamma_del_0
    ...
    # 根据WSet计算搜索令牌ST
    for kw in WSet:
        c, v, gamma_add_0, h, msk_c = state_client[kw]
        ...
        # 判断map_del中是否有kw
        if kw in map_del:
            ST.append([K_w, gamma_add_0,msk_c,map_del[kw]])
        else:
            # 随机生成一个穿刺链的gamma_del_0
            r=random.randint(-sys.maxsize-1,sys.maxsize)
            gamma_del_0=hmac.new(int(r).to_bytes(length=16,byteorder='big',signed='True')).digest()
            map_del[kw]=gamma_del_0
            ST.append([K_w, gamma_add_0,msk_c,map_del[kw]])
...
def update_client(K_e, K_s, new_kw_files, state_client, map_server,op,MSK_0,map_r,map_del):
    '''
    client端的更新操作
    ...
    state_client：client端的状态表，一个字典容器，储存kw->[c,v,gamma_add_0,h,msk]
    map_server：server端的储存容器，将addr映射到[P,V]
    op：操作类型。一个字符串，'add' or 'del'
    MSK_0：储存每个w的初始SPE主密钥 msk_0
    map_r：一个字典。储存所有叶结点对应的w的r值
    map_del：一个字典，储存w对应的被穿刺的gamma_del_0
    ...
    # 若w不存在r，那么这个函数会顺便随机生成r
    new_kw_files=construct_inverted_index(new_kw_files,map_r)




    # 对倒排索引new_kw_files中每个w-id pair，根据op类型，将其加入server或进行穿刺
    for kw in new_kw_files:
        ...
        if kw in state_client:
            ...
            gamma_add_0 = hmac.new(int(r_temp).to_bytes(
                length=16, byteorder='big', signed='True')).digest()
            # 生成SPE的密钥msk_0并更新MSK_0
            msk_c=SPE.KeyGen(128,5)       
            MSK_0[kw]=msk_c         
        ...
        if op=='add':
            ...
            for ind in new_kw_files[kw]:
                # 生成新的gamma
                r_temp = random.randint(-sys.maxsize-1, sys.maxsize)
                gamma_now = hmac.new(int(r_temp).to_bytes(
                    length=16, byteorder='big', signed='True')).digest()

                # 计算当前ind的地址addr=G(K_w,gamma_now)
                addr = AES.new(key=K_w, mode=AES.MODE_ECB).encrypt(gamma_now)

                # 计算pos=addr xor gamma_add_0
                pos = bytes(a ^ b for a, b in zip(addr, gamma_add_0))

                # 计算val=Enc(K_e,id||r)
                val = AES.new(key=K_e, mode=AES.MODE_ECB).encrypt(ind)
                ...
                # 更新gamma_add_0
                gamma_add_0=gamma_now


            # 更新状态表
            state_client[kw] = [c, v, gamma_add_0, h,msk_c]
        ...
        # op为delete
            ...
            for ind in new_kw_files[kw]:
                ...
                # 对t进行穿刺
                msk_c,psk=SPE.Pun(msk_c,t,c)

                # 随机生成gamma
                r=random.randint(-sys.maxsize-1,sys.maxsize)
                gamma_now=hmac.new(int(r).to_bytes(length=16,byteorder='big',signed='True')).digest()

                # 计算addr
                addr=AES.new(key=K_w,mode=AES.MODE_ECB).encrypt(gamma_now)


                # 判断map_del中是否有w对应的元素。若没有，需要初始化gamma_del_0
                if kw not in map_del:
                    r=random.randint(-sys.maxsize-1,sys.maxsize)
                    gamma_del_0=hmac.new(int(r).to_bytes(length=16,byteorder='big',signed='True')).digest()
                    map_del[kw]=gamma_del_0

                # 获取gamma_del_0
                gamma_del_0=map_del[kw]

                # 计算P
                pos=bytes(a^b for a,b in zip(addr,gamma_del_0))
                
                # 将(pos,psk,t)加入server
                map_server[addr]=[pos,psk,t]
