...
def test_scheme_honest(lb, ub, state_client, K_s, K_e, map_server, eth_contract, from_account,MSK_0,map_r,map_del):
    '''
...
def test_scheme_cheat(lb, ub, state_client, K_s, K_e, map_server, eth_contract, from_account,MSK_0,map_r,map_del):
    '''
...
def test_scheme_deny(lb, ub, state_client, K_s, K_e, map_server, eth_contract, from_account,MSK_0,map_r,map_del):
    '''
...
# 与标准答案对比
with open ("v1_case1_dump.txt", 'rb') as f: #打开文件
...
# 与标准答案对比
with open ("v1_case2_dump.txt", 'rb') as f: #打开文件
    standard = pickle.load(f)
if dec_set==standard:
    print("equal to standard")
...
######################################### 运行测试5，进行(w,ind) pair的删除 ###############################
print('start test case 5')

# 给出要删除的倒排索引
del_list={'1':['560','7'],'0':['0'],'4':['2']}
# 删除
checklist=client.update_client(K_e,K_s,del_list,state_client,map_server,'del',MSK_0,map_r,map_del)
# 将新的checklist发送至区块链
for l_w in checklist:
    tx_hash = eth_contract.functions.set(l_w, checklist[l_w]).transact({
        "from": from_account,
...
# 与标准答案对比
with open ("v1_case6_dump.txt", 'rb') as f: #打开文件
    standard = pickle.load(f)
if dec_set==standard:
    print("equal to standard")
print('finish test case 6')
...
# 与标准答案对比
with open ("v1_case7_dump.txt", 'rb') as f: #打开文件
    standard = pickle.load(f)
if dec_set==standard:
    print("equal to standard")
print('finish test case 7')
